...
class AsyncOpenAIBatchClient:
    ...
    async def upload(self, batch_input: Path) -> str:
        try:
            with batch_input.open("rb") as file:
                batch_input_file = await self.openai_client.files.create(
                    file=file,
                    purpose="batch",
                )
    ...
    async def get_batch_id(self, input_file_id: str) -> str:
        try:
    ...
    async def get_batch_info(self, batch_id: str) -> dict[str, Any]:
        try:
            batch = await self.openai_client.batches.retrieve(
                batch_id=batch_id,
            )
    ...
    async def get_batch_output(self, output_file_id: str) -> str:
        try:
            file_response = await self.openai_client.files.content(
    ...
    async def delete_uploaded_file(self, input_file_id: str) -> None:
        try:
            await self.openai_client.files.delete(file_id=input_file_id)
        except Exception as error:
            warnings.warn(f"Failed to delete uploaded file: {input_file_id}\n{error}")

    async def cancel(self, batch_id: str) -> None:
        try:
            await self.openai_client.batches.cancel(batch_id=batch_id)
        except Exception as error:
    ...
    async def submit(self, batch_input_file: OpenaiBatchInputFile) -> BatchJob:
        ...
        try:
            input_file_id = await self.upload(batch_input)
        ...
        try:
            batch_id = await self.get_batch_id(input_file_id)
            batch_job.update(batch_id=batch_id, status="submitted")
        except Exception as error:
            await self.delete_uploaded_file(input_file_id)
            raise RuntimeError(f"Failed to submit batch: {batch_input}") from error
    ...
    async def wait_for_completion(
        ...
        try:
            while True:
                try:
                    batch_info = await self.get_batch_info(batch_job.batch_id)
                    ...
                    if failed_count > max_retries:
                        raise RuntimeError(
                            f"Failed to retrieve batch status after {max_retries} retries."
                        ) from error
    ...
    async def clean_up_batch_job(self, batch_job: BatchJob) -> None:
        if batch_job.input_file_id:
            await self.delete_uploaded_file(input_file_id=batch_job.input_file_id)

    async def cancel_batch_job(self, batch_job: BatchJob) -> None:
        batch_job.update_batch_info(
            batch_info=await self.get_batch_info(batch_job.batch_id)
        )

        if batch_job.status in {"validating", "submitted", "in_progress"}:
            await self.cancel(batch_id=batch_job.batch_id)